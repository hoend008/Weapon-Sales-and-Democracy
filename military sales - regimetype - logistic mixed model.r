...
dep <- read_excel("SIPRI_regimetype_2022-12-16.xlsx")
...
dep$democracy_dictatorship = factor(dep$democracy_dictatorship)
dep$nato = factor(dep$nato)
dep$ex_soviet_state = factor(dep$ex_soviet_state)
dep$bought_weapons = factor(dep$bought_weapons)
#dep$evernato = ifelse(!is.na(dep$evernato), 1, 0)
#dep$evernato = factor(dep$evernato)
#dep$lag_value = lag(dep$value)
...
lmer_logistic_main <- function(){
	...
	model.7 <- glmer(bought_weapons~ 1 + year_from_zero + nato + ex_soviet_state + log(military_spending_value_dollars+1) + democracy_dictatorship + (1 + year_from_zero | code3), 
						data = dep, 
						family=binomial(link='logit'),
						nAGQ=0)
	summary(model.7)
